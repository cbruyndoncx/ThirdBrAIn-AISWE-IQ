 ...
 *      carries a scheme, so it can't collide with the computer's ws(s):// reading
...
export function resolveHubTarget(): HubTarget | null {
  ...
  if (envAddress && envAddress.trim() !== '') {
    ...
    if (!baseUrl) {
      console.error(
        `Invalid SLAYZONE_HUB_ADDRESS (expected host[:port], no scheme/path): ${envAddress}`
      )
      process.exit(1)
    }
  ...
  if (file) {
    ...
    if (!baseUrl) {
      console.error(`Warning: ignoring invalid hub URL in ${getHubConfigPath()} — using local app.`)
      return null
    }
