 ...
 * `slay computer mint`, which needs the identical "which hub, what credential"
...
function hubUsersCommand(): Command {
  ...
  users
    ...
    .action(async (email: string) => {
      ...
      // the two dangerous cases itself (last remaining account, computer service
...
export function hubCommand(): Command {
    ...
    .action(async (opts: { json?: boolean }) => {
      ...
      console.log(
        `${'NAME'.padEnd(nameW)}  ${'PORT'.padEnd(5)}  ${'PID'.padEnd(7)}  ` +
          `${'ROOT'.padEnd(rootW)}  ${'COMPUTERS'.padEnd(9)}  UPTIME`
      )
      console.log(
        `${'-'.repeat(nameW)}  ${'-'.repeat(5)}  ${'-'.repeat(7)}  ${'-'.repeat(rootW)}  ` +
          `${'-'.repeat(9)}  ${'-'.repeat(8)}`
      )
      // Which of these is THIS shell's? The sweep is machine-wide by design (that
      // is the point of multi-hub), so it legitimately lists hubs this invocation
      // does not belong to — including a developer's desktop app alongside a test
      // run's sidecar. Marking the caller's own hub is what makes the list safe to
      // act on; restricting the sweep would cap the feature instead.
      const ownRoot = process.env.SLAYZONE_ROOT?.trim()
        ? resolvePath(process.env.SLAYZONE_ROOT.trim())
        : null
      for (const h of hubs) {
        ...
        const mine = ownRoot && resolvePath(h.root) === ownRoot ? '  ← this shell' : ''
        console.log(
          `${h.name.padEnd(nameW)}  ${String(h.port).padEnd(5)}  ${String(h.pid).padEnd(7)}  ` +
            `${root.padEnd(rootW)}  ${String(h.computersConnected).padEnd(9)}  ` +
            `${formatUptime(h.uptimeMs)}${mine}`
        )
      }
    })
  ...
  cmd
    ...
    .action(
      async (
        ...
        // `deriveComputerHubUrl` then embeds `ws://127.0.0.1:<port>/computers` in every
        // join token it mints. Minting still succeeds — so the failure is silent: a
        // computer on another machine dials its OWN loopback and never connects.
        ...
        if (publicAddress !== undefined) {
          ...
          console.log(`Join tokens will point computers at wss://${publicAddress}/computers.`)
        ...
        if (publicAddress === undefined) {
          ...
          // token on a VPS, pastes it into a computer elsewhere, and gets no explanation
          ...
          console.log(
            `This hub is local-only: it binds loopback, so join tokens it mints point at\n` +
              `127.0.0.1 and only a computer on THIS machine can use them. For a hub other\n` +
  ...
  cmd
    ...
    .action(async (name: string) => {
      ...
      // bouncing a live hub drops its connected computers and pty sessions, which is
      ...
      // ONE ROOT, ONE HUB — the same guard `create` applies, because `start` is the
      // other way a hub is booted. The name check above cannot catch this: a hub
      // occupying this root under a DIFFERENT name (renamed, or started by hand)
      // slips past it, and starting anyway would put two writers on one
      // `<ROOT>/storage` SQLite file.
      const occupant = (await discoverAllHubs()).find((h) => h.root === root)
      if (occupant) {
        fail(
          `A hub is already running in ${shortenPath(root)} — "${occupant.name}" on port ` +
            `${occupant.port} (pid ${occupant.pid}). Starting "${name}" would put two hubs ` +
            `on the same store.`
        )
      }
  ...
  cmd
    ...
    .action(async (nameOrPort: string, opts: { upgrade?: boolean }) => {
      ...
      supervisorStop('hub', backend, name)
      // NEVER start a second hub because the first would not die.
      //
      // This return value used to be discarded, and the consequence was two live
      // hubs on ONE root: a hub created before its unit existed (or started by hand)
      // is not owned by the supervisor, so `supervisorStop` is a no-op on it; the
      // wait then timed out, the old process kept :51110, and the "restart" bound
      // :51111 from the port block. Two hubs sharing `<ROOT>/storage` means two
      // writers on one SQLite file, so this has to be fatal rather than a warning.
      if (!(await waitForHubGone(hub.port, 15_000))) {
        fail(
          `Hub "${name}" is still answering on port ${hub.port} after 15s, so restarting ` +
            `would leave TWO hubs sharing ${shortenPath(root)} — refusing.\n` +
            `It is not owned by ${backend} (started by hand, docker, or before it was ` +
            `registered). Stop that process first:\n` +
            `  kill ${hub.pid}\n` +
            `then \`slay hub start ${name}\`.`
        )
      }
  ...
  // enforces auth, every off-box command (`computer mint`, the whole REST surface)
