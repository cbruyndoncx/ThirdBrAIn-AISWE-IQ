...
async function main(): Promise<void> {
  ...
  // covered by hub-lifecycle.test.ts / computer-lifecycle.test.ts.
  ...
  try {
    ...
    setDistTags({ latest: '1.0.0', beta: '1.1.0-beta.1' })
    {
      ...
      check(
        'mixed outdated: issues exactly one batched npm install -g call',
        log.length === 1,
        log.join('|')
      )
      check(
    ...
    seed(npmRoot, '@slayzone/computer', '1.0.0')
    setDistTags({ latest: '1.0.0', beta: '1.2.0-beta.3' })
    {
      ...
      check(
        ...
        log.some((l) => l.includes('@slayzone/cli@1.2.0-beta.3')) &&
          log.some((l) => l.includes('@slayzone/computer@1.2.0-beta.3')),
        log.join('|')
      )
    }
    ...
    setDistTags({ latest: '1.1.0' })
    {
      ...
      check(
        'pnpm-only: reports via the pnpm manager column',
        stdout.some((l) => l.includes('pnpm')),
        stdout.join('\n')
      )
      check(
      ...
      check(
        ...
        log.some((l) => l.includes('@slayzone/cli@1.1.0')) &&
          log.some((l) => l.includes('@slayzone/hub@1.1.0')),
        log.join('|')
      )
    ...
    enableBun()
    clearInstallLogs()
    seed(bunGlobalNodeModules, '@slayzone/computer', '0.9.0') // native deps
    setDistTags({ latest: '1.0.0' })
    {
      const { stdout } = await runUpdate()
      const log = readLog(bunInstallLog)
      check(
        'bun-only: reports via the bun manager column',
        stdout.some((l) => l.includes('bun')),
        stdout.join('\n')
      )
      check(
        'bun-only: --trust is set for computer (native deps)',
        log.some((l) => l.includes('--trust')),
        log.join('|')
      )
      check(
        'bun-only: installs the outdated package at the target version',
        log.some((l) => l.includes('@slayzone/computer@1.0.0')),
        log.join('|')
      )
    }
    ...
    resetNpm()
    enableBun() // fresh bun root — the previous case's computer must not leak in here
    clearInstallLogs()
    ...
    setDistTags({ latest: '1.0.0' })
    {
      ...
      const log = readLog(bunInstallLog)
      check(
        'bun-only, cli: no --trust flag (no native deps)',
        !log.some((l) => l.includes('--trust')),
        log.join('|')
      )
    }
