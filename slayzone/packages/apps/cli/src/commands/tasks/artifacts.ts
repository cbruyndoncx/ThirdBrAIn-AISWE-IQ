...
/*
 * Hub/computer split (Wave 3.5): the subcommands below route through api.ts — the
...
export function artifactsSubcommand(): Command {
  ...
  cmd
    ...
    .action(async (artifactId: string, opts: { mutateVersion?: boolean | string }) => {
      ...
      console.log(`Written: ${data.id.slice(0, 8)}  ${data.title}  v${data.version.version_num}`)
    })
  ...
  cmd
    ...
    .action(async (artifactId: string, opts: { mutateVersion?: boolean | string }) => {
      ...
      console.log(`Appended: ${data.id.slice(0, 8)}  ${data.title}  v${data.version.version_num}`)
    })
...
`
    ...
    .action(async (artifactId: string | undefined, opts) => {
      ...
      if (opts.type === 'zip') {
        ...
        if (opts.json) {
          console.log(
            JSON.stringify({ path: outputPath, type: 'zip', taskId: artifacts[0].task_id })
          )
        } else {
      ...
      if (opts.type === 'raw') {
        ...
        fs.mkdirSync(path.dirname(outputPath), { recursive: true })
        await pipeline(
          Readable.fromWeb(body as StreamWebReadable),
