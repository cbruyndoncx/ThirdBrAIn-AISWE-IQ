/**
 * Computer restart-survival: local computer stays ONE row across reboots.
 *
 * The gap the single-boot computer e2e (110-computer-loopback) missed: nothing proved
 * a SECOND boot against the SAME persisted store reuses the computer's row instead
 * of orphaning a fresh one. The computer keys its credential entry (one shared
 * `computer.state.json` map, entries keyed by hub host+PORT via `hubHostFromUrl`
 * → `host_port`) by hub host+PORT, so if the port CHANGES between boots the
 * credential key changes ⇒ the computer can't `hello` with prior creds ⇒
 * RE-ENROLLS as a new `computers` row (an orphaned `local-computer` per relaunch).
 *
 * Single-listener model: `/computers` rides the ONE hub port (no separate computer
 ...
 * separate computer-port persistence layer to keep in sync. This test proves the
 ...
 *   - `deriveComputerHubUrl` (the shared-port URL the join token embeds).
 *   - the computer's REAL `createFileCredentialStore` keyed by `hubHostFromUrl`.
 *   - the REAL `createComputerAuthAdapters` (enroll/hello + local dedup) over a REAL
 *     `createHubAuth` + REAL `mintJoinToken` / api-key mint+verify.
 *
 * A `simulateBoot` models exactly one sidecar boot: derive the computer URL from the
 * (stable) hub port, mint a token embedding it, then run the computer's authenticate
...
import { listComputers, mintJoinToken } from '@slayzone/computers/server'
...
const LOCAL_NAME = 'local-computer'
/** The stable per-environment hub port `/computers` rides (SIDECAR_FIXED_PORT.test).
...
interface BootDeps {
  db: SlayzoneDb
  auth: HubAuth
  /** The computer's credential dir — STABLE across boots (holds one shared
   *  computer.state.json; the entry key within it varies, keyed by hub host+port
   *  via hubHostFromUrl). */
  credsBaseDir: string
  ...
  hubPort?: number
  /** Match production: the auth adapters know the local computer's name. */
...
/** One sidecar boot + one computer (re)connect against the SAME persisted store.
 *  Mirrors server.ts: derive the `/computers` URL from the (stable) hub port, mint a
 *  token embedding it, then run the computer's authenticate logic. */
async function simulateBoot(deps: BootDeps): Promise<BootResult> {
  const hubUrl = deriveComputerHubUrl({
    remote: false,
    host: HOST,
    port: deps.hubPort ?? HUB_PORT
  })!
  ...
  // The computer side: real credential store keyed by hub host+port, real adapters.
  const store = createFileCredentialStore(hubHostFromUrl(hubUrl), { baseDir: deps.credsBaseDir })
  const adapters = createComputerAuthAdapters({
    db: deps.db,
    auth: deps.auth,
    ...(deps.localComputerName ? { localComputerName: deps.localComputerName } : {})
  })
  ...
  if (stored) {
    ...
    if (descriptor) {
      return { hubUrl, authMode: 'hello', computerId: descriptor.computerId }
    }
  }
  ...
  await store.save({ computerId: enrolled.computerId, apiKey: enrolled.apiKey })
  return { hubUrl, authMode: 'enroll', computerId: enrolled.computerId }
}

function localComputerCount(db: SlayzoneDb): Promise<number> {
  return listComputers(db).then((rows) => rows.filter((r) => r.name === LOCAL_NAME).length)
}
...
async function main(): Promise<void> {
  ...
  const tmpDir = mkdtempSync(join(tmpdir(), 'computer-restart-'))
  const auth = await createHubAuth({
    dbPath: join(tmpDir, 'hub-auth.sqlite'),
    baseURL: 'http://127.0.0.1:9998',
    secret: 'computer-restart-survival-secret-at-least-32-chars'
  })
  ...
  try {
    await test('stable hub port ⇒ boot 2 HELLOs into the same row (count stays 1)', async () => {
      ...
      try {
        const boot1 = await simulateBoot({ db, auth, credsBaseDir, localComputerName: LOCAL_NAME })
        assertEq(boot1.authMode, 'enroll', 'boot 1 enrolls (no prior creds)')
        assertEq(await localComputerCount(db), 1, 'one local computer after boot 1')

        // Boot 2: same stable hub port → identical credential key → hello.
        const boot2 = await simulateBoot({ db, auth, credsBaseDir, localComputerName: LOCAL_NAME })
        assertEq(boot2.hubUrl, boot1.hubUrl, 'boot 2 derives the identical computer URL')
        assertEq(boot2.authMode, 'hello', 'boot 2 HELLOs with the persisted creds')
        assertEq(boot2.computerId, boot1.computerId, 'same computerId across reboots')
        assertEq(await localComputerCount(db), 1, 'STILL one local computer after boot 2')

        // A third boot for good measure — the survival property must hold steady.
        const boot3 = await simulateBoot({ db, auth, credsBaseDir, localComputerName: LOCAL_NAME })
        assertEq(boot3.authMode, 'hello', 'boot 3 also HELLOs')
        assertEq(await localComputerCount(db), 1, 'still one local computer after boot 3')
      } finally {
        close()
      }
    })
    ...
    await test('regression guard: a CHANGED hub port would change the credential key (why stability matters)', async () => {
      ...
      // the hub port differed between boots, the computer's credential key
      ...
      const { close } = createMigratedDb()
      const credsBaseDir = mkdtempSync(join(tmpDir, 'creds-portchange-'))
      try {
        const url1 = deriveComputerHubUrl({ remote: false, host: HOST, port: 60000 })!
        const url2 = deriveComputerHubUrl({ remote: false, host: HOST, port: 60001 })!
        ...
        await store1.save({ computerId: 'x', apiKey: 'k' })
    ...
    await test('PART 2 dedup collapses pre-existing duplicates to one on next local enroll', async () => {
      ...
      // legitimate REMOTE computer that must survive.
      const { db, close } = createMigratedDb()
      const credsBaseDir = mkdtempSync(join(tmpDir, 'creds-dedup-'))
      try {
        const remoteAdapters = createComputerAuthAdapters({ db, auth }) // no localComputerName → fresh uuids
        for (const label of ['orphan-1', 'orphan-2', 'orphan-3']) {
          const t = await mintJoinToken(db, {
            hubUrl: `wss://${HOST}:59999/computers`,
            certFingerprint: 'sha256:deadbeef',
            ttlMs: 60_000,
            label
          })
        ...
        // A real remote computer (different name) — a sleeping laptop; must NOT be reaped.
        ...
        assertEq(await localComputerCount(db), 3, 'three orphaned local rows seeded')

        // The next computer-enabled boot: the local computer enrolls → dedup collapses.
        const boot = await simulateBoot({ db, auth, credsBaseDir, localComputerName: LOCAL_NAME })
        assertEq(boot.authMode, 'enroll', 'first boot in this store enrolls')
        assertEq(await localComputerCount(db), 1, 'collapsed to a single local computer')
        // Remote computer untouched — dedup is identity-only (by local name), never
        // status-based and never touches a remote computer.
        assert(
          (await listComputers(db)).some((r) => r.id === remote.computerId),
          'remote computer survives'
        )
      } finally {
        close()
      }
