 ...
 * the computer's `ws(s)://…/computers` can never disagree about it.
 *
 ...
 * prefix today — deriveComputerHubUrl forces `/computers` and this base URL is only
...
/**
 * Build the remote-MCP-env provider the composition root injects under computer
 * mode (hub/computer split, wave 3.5). Extracted from the composition root so it's
 * a pure, directly-testable function: given a task + computer it resolves the
 * hub's externally-reachable base URL (used ONLY by the `slay` CLI's hub REST
 * access — the agent HOOK posts to the computer's own loopback relay, not here).
 *
 ...
 *     SLAYZONE_MODE) — REQUIRED for a truly-remote computer, which can't reach the
 *     hub's loopback.
 *   - else `http://127.0.0.1:<boundPort>` — reachable only by a co-located /
 *     loopback computer; the safe default for a local computer.
 ...
 *
 * No per-task bearer is minted: the agent lifecycle hook posts to the computer's
...
export function createRemoteMcpEnvProvider(opts: {
  ...
  return ({ computerId }) => {
    ...
    // from a truly-remote computer) would be worse than degrading via the null path.
