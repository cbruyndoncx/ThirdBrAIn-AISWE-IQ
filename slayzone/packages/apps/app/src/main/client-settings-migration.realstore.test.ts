...
if (!fixture) {
  console.log('  — skipped: set SLAYZONE_REAL_STORE_FIXTURE to a real slayzone.sqlite to run this')
  ...
  for (const suffix of ['-wal', '-shm']) {
    if (fs.existsSync(`${fixture}${suffix}`))
      fs.copyFileSync(`${fixture}${suffix}`, `${copy}${suffix}`)
  }
