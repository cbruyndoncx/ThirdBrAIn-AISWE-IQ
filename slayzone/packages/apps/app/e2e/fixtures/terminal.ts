...
/**
 * Structural view of the xterm `Terminal` reachable through the link provider
 * that `xterm-init.ts` publishes on `window.__slayzone_terminalLinks` — only the
 * buffer/geometry members the readers below touch. Spelled out here rather than
 * imported from `@xterm/xterm` because the terminal only exists inside
 * `page.evaluate`, in the app's renderer, never in this Node-side process.
 */
type XtermBufferLine = { translateToString(trimRight?: boolean): string }
type XtermProbe = {
  rows: number
  buffer: {
    active: {
...
type TerminalLinksWindow = Window & {
  __slayzone_terminalLinks?: Record<string, { _terminal: XtermProbe } | undefined>
}
...
export async function waitForPtyState(
  ...
  await expect
    .poll(
      async () =>
        page.evaluate(
          (id) => window.getTrpcVanillaClient().pty.getState.query({ sessionId: id }),
          sessionId
        ),
      {
        timeout: timeoutMs
...
/**
 * Write a command + CR into a pty and WAIT for the write to be acknowledged.
 *
 * The `return` is load-bearing. Without it the arrow's block body discards the
 * mutate promise, so `page.evaluate` resolves as soon as the synchronous block
 * returns `undefined` — i.e. before the write has been dispatched, let alone
 * received by the pty. Every caller that writes and then immediately asserts was
 * racing an unsynchronised write, and the gap widens under machine load. A scan
 * of the whole e2e tree found this was the only dropped tRPC promise in it.
 */
...
export async function getViewportLines(page: Page, sessionId: string): Promise<string[] | null> {
  return page.evaluate(
    ({ sid }) => {
      const links = (window as TerminalLinksWindow).__slayzone_terminalLinks
