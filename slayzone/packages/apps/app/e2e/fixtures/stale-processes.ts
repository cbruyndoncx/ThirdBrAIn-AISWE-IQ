 ...
 * so a path match cannot tell them apart. Matching `computer/dist/bin.cjs`
 * therefore SIGTERM'd the supervised dev app's local computer, which owns every
 ...
 * every e2e app with `PLAYWRIGHT=1`, and the sidecar + local computer it spawns
 ...
 * orphan costs a warning line; a killed dev computer costs every running agent.
 */
