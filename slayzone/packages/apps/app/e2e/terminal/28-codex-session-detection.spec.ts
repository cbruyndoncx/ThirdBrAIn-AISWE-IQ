...
test.describe('Session ID banners', () => {
  ...
  const openBannerTask = async (
    ...
    await expect(async () => {
      ...
      // Strictly below the 30s test timeout: at exactly 30s this wrapper could never
      // report its own failure, so a real miss surfaced as "banner not visible,
      // 4000ms" (the inner assertion) instead of the 30s budget it actually spent.
  ...
  test.beforeAll(async ({ electronApp, mainWindow }) => {
    ...
    // Opened by title, never by id — it stays on the default claude-code mode,
    // so nothing below has to address it through a task update.
    await s.createTask({
      projectId: p.id,
