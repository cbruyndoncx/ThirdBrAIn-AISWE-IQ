...
/** Satisfies required app-chrome callbacks the fork has no native window to drive. */
const NOOP = (): void => {
...
export function HomeView(): React.JSX.Element {
  ...
  if (boardStatus === 'error') {
    return (
      <Centered>
        Couldn’t load the board: {boardError?.message ?? 'unknown error'}. Retrying…
      </Centered>
    )
  }
  if (boardStatus === 'pending') {
    return (
      <Centered>
        {stalled ? 'Can’t reach the sidecar — is the server running?' : 'Connecting…'}
      </Centered>
    )
  }
  ...
  return (
    <SidebarProvider defaultOpen className="h-svh min-h-0 bg-background text-foreground">
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        ...
        <div
          ref={explodeGridRef}
          className={cn('relative min-h-0 flex-1 overflow-hidden', explodeMode && 'grid gap-1 p-1')}
          style={
            explodeMode
              ? (() => {
