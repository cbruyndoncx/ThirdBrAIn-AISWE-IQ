...
// The input arrives untyped off the wire and every field is forwarded straight
// into an `AnyFn` (which takes `unknown` args), so an open record of `unknown`
// is as precise as the bridge can be without re-deriving each proc's zod input.
type ProcInput = Record<string, unknown>
type Handler = (h: BrowserHost, i: ProcInput) => unknown
    ...
    (opts: {
      ...
      return observable<Envelope, TRPCClientError<AppRouter>>((observer) => {
        ...
        if (!host) {
          observer.error(
            TRPCClientError.from(
              new Error(`[browser-mojo-link] window.api.browser unavailable for ${op.path}`)
            )
          )
          return
        }
        ...
        if (op.type === 'subscription') {
          ...
          try {
            unsub = sub((evt: unknown) =>
              observer.next({ result: { type: 'data', data: evt } })
            ) as () => void
          } catch (err) {
        ...
        if (!handler) {
          observer.error(
            TRPCClientError.from(new Error(`[browser-mojo-link] unmapped app.browser.${method}`))
          )
          return
        }
        Promise.resolve()
          .then(() => handler(host, op.input as ProcInput))
          .then((data) => {
