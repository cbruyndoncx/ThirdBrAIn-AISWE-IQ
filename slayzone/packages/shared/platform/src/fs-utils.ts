 ...
 * installer exists on the same machine (a standalone computer), because the two can
  ...
  for (;;) {
    try {
      ...
      if (Date.now() > deadline) {
        throw new Error(`[fs-utils] timed out after ${LOCK_TIMEOUT_MS}ms waiting for ${lockPath}`)
      }
