...
export function registerArtifactsCrudRoutes(app: Express, deps: RestApiDeps): void {
  ...
  app.get('/api/artifacts/:id', async (req, res) => {
    try {
      ...
      res.json({
        ...
        data: {
          ...row,
          filePath: resolveArtifactFilePath(String(row.task_id), row.id, String(row.title ?? ''))
        }
      })
    } catch (err) {
  ...
  app.post('/api/artifacts', async (req, res) => {
    ...
    try {
      ...
      const artifact = await store().createArtifact(db, {
        ...
        folderId,
        renderMode:
          typeof body.renderMode === 'string' ? (body.renderMode as RenderMode) : undefined,
  ...
  app.patch('/api/artifacts/:id', async (req, res) => {
    ...
    try {
      ...
      if (body.renderMode !== undefined) {
        input.renderMode =
          typeof body.renderMode === 'string' ? (body.renderMode as RenderMode) : null
      }
  ...
  app.post('/api/artifact-folders', async (req, res) => {
    ...
    try {
      ...
      const folder = await store().createFolder(db, {
        taskId: task.row.id,
        name: body.name,
        parentId
      })
