...
export const settingsRouter = router({
  get: publicProcedure
    ...
    .query(
      async ({ ctx, input }) => (await SettingsService.forDatabase(ctx.db).get(input.key)) ?? null
    ),
  ...
  setTheme: publicProcedure.input(themeInput).mutation(({ input }) => getAppDeps().themeSet(input)),
