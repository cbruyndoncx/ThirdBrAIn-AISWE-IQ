...
test('backfills sessions + reset, preserves id/created_at, drops the manual-reset session', () => {
  ...
  try {
    ...
    seedConv(db, {
      ...
      pendingMeta: '{"usedResume":false,"spawnedAt":1005}',
      createdAt: 1005
    })
    ...
    const sessCount = (
      db
        .prepare(
          "SELECT count(*) AS n FROM agent_sessions WHERE task_id='t1' AND mode='claude-code'"
        )
        .get() as { n: number }
    ).n
    ...
    const resetCount = (
      db
        .prepare(
          "SELECT count(*) AS n FROM session_resets WHERE task_id='t1' AND mode='claude-code'"
        )
        .get() as { n: number }
    ).n
    expectEqual(resetCount, 1)
    const resetInSessions = (
      db.prepare("SELECT count(*) AS n FROM agent_sessions WHERE origin='manual-reset'").get() as {
        n: number
      }
    ).n
    ...
    const c1 = db
      .prepare(
        "SELECT id, conversation_id, origin, status, cwd, created_at, bound_at FROM agent_sessions WHERE id='r1'"
      )
    ...
    const reset = db
      .prepare("SELECT id, task_id, mode, created_at FROM session_resets WHERE id='r4'")
      .get() as Record<string, unknown>
...
test('empty ledger → empty agent tables, resolver returns null', () => {
  ...
  try {
    ...
    expectEqual(
      (db.prepare('SELECT count(*) AS n FROM agent_sessions').get() as { n: number }).n,
      0
    )
    expectEqual(
      (db.prepare('SELECT count(*) AS n FROM session_resets').get() as { n: number }).n,
      0
    )
