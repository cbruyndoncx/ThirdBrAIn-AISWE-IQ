...
export const appShim = {
  ...
  // Same for the computer: the fork does not spawn it, so it cannot cycle it.
  restartLocalComputer: async (): Promise<{ ok: boolean; error?: string }> => ({
    ok: false,
    error: 'Not supported in the Chromium shell'
  }),
