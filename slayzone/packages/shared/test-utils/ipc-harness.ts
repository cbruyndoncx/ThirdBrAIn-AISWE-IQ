...
const fakeEvent = {
  sender: {
    send: () => {
      // Handlers that push to the renderer get a sink: there is no renderer here,
      // and contract tests assert on the handler's return value and the resulting
      // DB state, never on the push.
    }
  }
}
...
> {
  ...
  const registryPath = path.resolve(import.meta.dirname, '../transport/src/txns/index.ts')
...
// Minimal test computer — chains async tests sequentially, runs sync tests immediately
