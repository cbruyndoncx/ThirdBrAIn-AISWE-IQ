...
export function SkillsSection({ level, projectId, projectPath }: SkillsSectionProps) {
  ...
  useEffect(() => {
    trpcClient.aiConfig.marketplace.checkUpdates
      ...
      .catch(() => {
        // Update badges are decoration on top of the installed skills: an
        // unreachable or rate-limited marketplace leaves the list rendering
        // exactly as before, so it must not raise anything at the user.
      })
  }, [trpcClient, items])
  ...
  const handleUnlink = useCallback(
    async (target: AiConfigItem) => {
      ...
      if (hasMarketplace) {
        const updated = await trpcClient.aiConfig.marketplace.unlinkSkill.mutate({
          itemId: target.id
        })
        if (updated)
          setItems((prev) => prev.map((i) => (i.id === updated.id ? (updated as AiConfigItem) : i)))
