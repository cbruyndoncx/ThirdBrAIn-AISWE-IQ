...
export function useSkillMarketplace(projectId: string | null, projectPath?: string | null) {
  ...
  useEffect(() => {
    trpcClient.aiConfig.marketplace.ensureFresh
      .mutate()
      .then(() => {
        setRefreshKey((k) => k + 1)
      })
      .catch(() => {
        // Best-effort freshness pass: on failure the already-loaded registry
        // rows stay on screen and the user can still refresh by hand.
      })
  }, [trpcClient])
