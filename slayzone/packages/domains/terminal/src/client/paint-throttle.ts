...
export function decideThrottle(
  ...
  const nextFloodScore = (1 - opts.floodEmaAlpha) * state.floodScore + opts.floodEmaAlpha * bytes
  const flooding = nextFloodScore > opts.floodBytesThreshold || bytes > opts.floodBytesThreshold
