...
import type { TerminalMode, TerminalModeInfo, UsageProviderConfig } from '@slayzone/terminal/shared'
import { buildMcpEnv as buildMcpEnvDb, type RemoteMcpEnv } from '../mcp-env'

/**
 * Hub/computer split (wave 1): the PTY runtime's DB touchpoints, pulled behind
 ...
 * injection seam when the computer actually loses its local DB.
 */
...
export interface PtySessionLedger {
   ...
   *
   * `remote` (hub/computer split, wave 3): a resolved hub target when this
   * session's pty runs on a computer — replaces the loopback hook URL/MCP port
...
export function createDbPtySessionLedger(db: SlayzoneDb): PtySessionLedger {
  return {
    ...
    buildMcpEnv: (taskId, mode, remote) =>
      buildMcpEnvDb(db, taskId, mode, undefined, undefined, remote)
  }
}
...
export interface PtySpawnLookups {
  ...
  /**
   * Hub/computer split (wave 2, Model A): the computer a task's PTY should spawn on,
   * or `null` for hub-local (today's only path). The db default ALWAYS returns
   * null; a later wave injects a computer-aware impl via `setPtySpawnLookups` so
   * `ptyCreate` can route the spawn (and gate warm-pool adoption to local).
   */
  resolveComputerId(taskId: string): Promise<string | null>
}

/**
 * A `terminal_modes` row exactly as SQLite hands it back: snake_case columns,
 * booleans still 0/1, and `usage_config` still a JSON string. `mapModeRow` is
 * the one place that shape is known, so it stays local to this module.
 */
interface TerminalModeRow {
  id: string
  label: string
  type: string
  ...
  is_builtin: number
  order: number
  pattern_working: string | null
  pattern_error: string | null
  usage_config: string | null
}

/**
 * Map a raw `terminal_modes` row to the shared TerminalModeInfo shape.
 *
 * Takes `unknown` because every caller feeds it straight from `db.prepare(...)`,
 * whose result the DB bridge types as `unknown`; the row shape is asserted once
 * here rather than at each of the five call sites.
 */
export function mapModeRow(row: unknown): TerminalModeInfo {
  const r = row as TerminalModeRow
  let usageConfig: UsageProviderConfig | null = null
  if (r.usage_config) {
    try {
      usageConfig = JSON.parse(r.usage_config) as UsageProviderConfig
    } catch {
      /* ignore corrupt */
    }
  }
  return {
    ...
    label: r.label,
    type: r.type,
    initialCommand: r.initial_command,
    resumeCommand: r.resume_command,
    headlessCommand: r.headless_command ?? null,
    defaultFlags: r.default_flags,
    enabled: Boolean(r.enabled),
    isBuiltin: Boolean(r.is_builtin),
    order: r.order,
    patternWorking: r.pattern_working,
    patternError: r.pattern_error,
    usageConfig
  }
}
...
export function createDbPtySpawnLookups(db: SlayzoneDb): PtySpawnLookups {
  return {
    ...
    getTaskProjectId: async (taskId) => {
      const row = (await db.prepare('SELECT project_id FROM tasks WHERE id = ?').get(taskId)) as
        | { project_id?: string }
        | undefined
      return row?.project_id ?? null
    },
    // Hub-local by default — no computer assignment exists yet. Wave 2's
    // computer-aware lookups impl overrides this via `setPtySpawnLookups`.
    resolveComputerId: async () => null
  }
}
