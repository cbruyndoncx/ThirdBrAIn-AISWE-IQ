...
export function parseArtifact(row: Record<string, unknown> | undefined): TaskArtifact | null {
  ...
  return {
    ...
    html_theme_override: (row.html_theme_override as 'light' | 'dark' | null) ?? null,
...
export function createArtifactStore(dataDir: string) {
  ...
  return {
    ...
    async updateArtifact(
      ...
      const row = await db.namedTxn('task-artifacts:update', {
        dataDir,
        id: data.id,
        ...
        htmlThemeOverride: data.htmlThemeOverride,
    ...
    searchArtifacts(db: SlayzoneDb, data: ArtifactSearchOptions): Promise<ArtifactSearchReport> {
    ...
    async getOrCreateFolderByName(
      ...
      const row = await db.namedTxn('task-artifacts:folders:getOrCreateByName', {
        taskId: data.taskId,
        name: data.name
      })
    ...
    async createFolder(
      ...
      const row = await db.namedTxn('task-artifacts:folders:create', {
        taskId: data.taskId,
    ...
    async updateFolder(
      ...
      const existing = (await db
        .prepare('SELECT * FROM artifact_folders WHERE id = ?')
