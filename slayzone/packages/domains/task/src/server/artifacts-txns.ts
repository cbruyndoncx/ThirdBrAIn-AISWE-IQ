...
export interface UpdateArtifactTxnParams {
  ...
  zoomPct?: number | null
  htmlThemeOverride?: 'light' | 'dark' | null
  language?: string | null
  content?: string
...
export const artifactsTxns = {
  ...
  'task-artifacts:update': (db: Database, p: UpdateArtifactTxnParams): Row | null => {
    ...
    if (has('htmlThemeOverride')) {
      sets.push('html_theme_override = ?')
      values.push(p.htmlThemeOverride)
    }
    if (has('language')) {
      sets.push('language = ?')
   ...
   */
  'task-artifacts:search': (db: Database, p: VersionsSearchTxnParams): ArtifactSearchReport =>
    wrapVersionError(() =>
