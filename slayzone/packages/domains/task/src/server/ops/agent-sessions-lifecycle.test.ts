...
async function main(): Promise<void> {
  ...
  assert(
    (await getCurrentConversationId(db, 't1', 'claude-code')) === 'S1',
    'fresh becomes current after its first turn'
  )

  // 2. Write-once: a second confirm is a no-op (origin no longer pending).
  const o1b = await confirmSessionConversation(db, {
    sessionId: 'S1',
    observedConversationId: 'HIJACK'
  })
  assert(o1b === null, 'second confirm is a no-op (write-once)')
  assert(
    (await getCurrentConversationId(db, 't1', 'claude-code')) === 'S1',
    'conversation_id unchanged after re-confirm'
  )
  ...
  const o3 = await confirmSessionConversation(db, {
    sessionId: 'S3',
    observedConversationId: 'FOREIGN'
  })
  assert(o3 === 'foreign-observed', `mismatch → foreign, got ${o3}`)
  assert(
    (await getCurrentConversationId(db, 't1', 'claude-code')) === 'S1',
    'foreign not honored — last honored (S1) wins'
  )
  ...
  const oc = await confirmSessionConversationByTaskMode(db, {
    taskId: 't1',
    mode: 'codex',
    observedConversationId: 'CDX-9'
  })
  ...
  assert(
    (await getCurrentConversationId(db, 't1', 'codex')) === 'CDX-9',
    'codex conversation honored'
  )
  ...
  assert(
    ...
    'dead session not returned as in-flight pending'
  )
  const s4 = raw.prepare("SELECT status, ended_at FROM agent_sessions WHERE id='S4'").get() as {
    status: string
    ended_at: number | null
  }
  assert(
    s4.status === 'dead' && typeof s4.ended_at === 'number',
    'markSessionDead sets status + ended_at'
  )

  // 7. Reset cutoff hides earlier honored conversations.
  raw
    .prepare(
      "INSERT INTO session_resets (id, task_id, mode, created_at) VALUES ('r1','t1','claude-code',?)"
    )
    .run(Date.now())
  ...
  assert(
    ...
    'pooled session not bound to any task'
  )
  ...
  assert(
    (await getCurrentConversationId(db, 't3', 'claude-code')) === null,
    'session did not move to t3'
  )
