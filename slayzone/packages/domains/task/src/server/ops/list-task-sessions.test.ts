...
async function main(): Promise<void> {
  ...
  insSession({
    ...
    origin: 'pending-spawn',
    createdAt: clk++
  }) // pending w/ pre-minted id, died pre-confirm
  insSession({
    id: 'f1',
    taskId: T,
    mode: M,
  ...
  assert(
    sessions[0].conversationId === 'B',
    `newest first: expected B, got ${sessions[0].conversationId}`
  )
  assert(
    sessions[1].conversationId === 'A',
    `oldest last: expected A, got ${sessions[1].conversationId}`
  )
  ...
  assert(
    a.lastActiveAt === 1002,
    `A lastActiveAt expected 1002 (last resume), got ${a.lastActiveAt}`
  )

  // 5b. origin = the FIRST spawn's origin (fresh), NOT a later resume row.
  assert(
    a.origin === 'slay-spawned-fresh',
    `A origin expected fresh (first spawn), got ${a.origin}`
  )
  ...
  raw
    .prepare(`INSERT INTO session_resets (id, task_id, mode, created_at) VALUES ('r1', ?, ?, ?)`)
    .run(T, M, clk++)
  const afterReset = await listTaskSessions(db, T, M)
  assert(
    afterReset.every((s) => s.isCurrent === false),
    'after reset, no session is current'
  )
