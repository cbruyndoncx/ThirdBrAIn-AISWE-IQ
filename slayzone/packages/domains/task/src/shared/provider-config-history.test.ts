...
cfg = appendProviderConversationId(cfg, M, 'a')
assert(
  JSON.stringify(hist(cfg)) === JSON.stringify(['b', 'c', 'a']),
  're-append moves id to end, deduped'
)
...
assert(
  cappedHist[cappedHist.length - 1] === `id-${CONVERSATION_HISTORY_CAP + 4}`,
  'cap keeps newest'
)
