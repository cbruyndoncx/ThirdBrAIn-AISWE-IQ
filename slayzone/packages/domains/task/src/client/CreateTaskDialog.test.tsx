...
vi.mock('@slayzone/transport/client', () => ({
  ...
  useClientForProject: (projectId?: string) =>
    hubIdFor(projectId) === 'remote-a' ? remote : ambient,
...
vi.mock('@slayzone/ui', () => {
  ...
  return {
    ...
    FormControl: pass,
    FormMessage: () => null,
    FormField: ({ control, name, render }: React.ComponentProps<typeof Controller>) => (
      <Controller control={control} name={name} render={render} />
    ),
    Input: (props: React.ComponentProps<'input'>) => <input {...props} />,
    Textarea: (props: React.ComponentProps<'textarea'>) => <textarea {...props} />,
    // `variant`/`size` are styling-only in the real Button — dropped here so
