...
export function resolveProjectDrop(input: DropInput): DropAction {
  ...
  if (active.isGroup) {
    ...
    return {
      type: 'reorderTop',
      refs: insertRef(topLevel, { kind: 'group', id: active.id }, overRef, mode)
    }
  }
  ...
  if (over.kind === 'group') {
    ...
    if (active.group !== null) {
      ...
      return {
        type: 'moveOut',
        projectId: active.id,
        index: Math.max(0, mode === 'after' ? i + 1 : i)
      }
    }
    return {
      type: 'reorderTop',
      refs: insertRef(
        topLevel,
        { kind: 'project', id: active.id },
        { kind: 'group', id: over.id },
        mode
      )
    }
  }
  ...
  if (tGroup !== null) {
    ...
    if (mode === 'merge') {
      ...
      return {
        ...
        groupId: tGroup,
        projectId: active.id,
        index: Math.max(0, list.indexOf(over.id))
      }
    }
