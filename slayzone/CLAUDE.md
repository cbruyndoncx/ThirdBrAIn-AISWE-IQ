...
4. One concept = one var, even when its value differs per role (the hub BINDS `SLAYZONE_HUB_ADDRESS`, the computer/CLI DIAL it). Safe because `sanitizeSpawnEnv` strips non-`global` vars at every terminal-spawn boundary.
...
6. `_URL` (full URL incl. path) allowed ONLY when the target is loopback-only AND the scheme is therefore invariant `http` — no MODE-derived scheme exists to get wrong, so rule 5 has nothing to protect. Today the sole case is `SLAYZONE_AGENT_HOOK_URL`: local = sidecar loopback, remote = the computer's OWN loopback (the computer relays to the hub over its authed ws), so the agent env is byte-identical local vs remote and carries no bearer. It stays `_URL` because its only reader is the deliberately logic-free shared `notify.sh` — an `_ADDRESS` form would push scheme+path assembly into that file, which is the exact rot the v3 benign-forwarder rewrite removed. `env-manifest.test.ts` asserts it is the only `_URL` key, so a new one fails loudly.
