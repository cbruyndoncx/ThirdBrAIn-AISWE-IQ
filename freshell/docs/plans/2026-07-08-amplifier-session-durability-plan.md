...
| E2/E3 | **`prompt:complete` is the single turn boundary.** Exactly one `execution:start`/`execution:end` pair per turn; tool loops are `provider:request` iterations *inside* it. But background session-naming `llm:request` / `provider:retry` events fire **after** `prompt:complete`. | Reducer must never interpret post-complete events as a new turn. **Only `prompt:submit` re-enters busy.** *Amended 2026-08-10: `orchestrator:complete` (root, null `parent_id`) is also a turn boundary — provider-error turns never write `prompt:complete`.* |
